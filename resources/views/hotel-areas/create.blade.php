...
@section('title', 'Create Hotel Area')
@section('page-title', 'Create Hotel Area')
...
<div style="background: white; border-radius: 12px; padding: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <form method="POST" action="{{ route('hotel-areas.store') }}">
        @csrf

        <div class="form-group">
            <label for="name">Area Name *</label>
            <input type="text" id="name" name="name" value="{{ old('name') }}" required placeholder="e.g., Reception, Lobby, Pool Area">
            @error('name')
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" rows="3" placeholder="Area description...">{{ old('description') }}</textarea>
            @error('description')
                <span class="error">{{ $message }}</span>
            @enderror
        </div>
