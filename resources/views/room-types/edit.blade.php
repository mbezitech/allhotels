...
<div style="background: white; border-radius: 12px; padding: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <form method="POST" action="{{ route('room-types.update', $roomType) }}">
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" rows="3">{{ old('description', $roomType->description) }}</textarea>
        </div>

        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="form-group">
                <label for="base_price">Base Price Per Night *</label>
                <input type="number" id="base_price" name="base_price" value="{{ old('base_price', $roomType->base_price) }}" step="0.01" min="0" required>
                @error('base_price')
                    <span class="error">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" id="default_capacity" name="default_capacity" value="{{ old('default_capacity', $roomType->default_capacity) }}" min="1" required>
        ...
        <div class="form-group">
            <div class="checkbox-group">
                <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $roomType->is_active) ? 'checked' : '' }}>
